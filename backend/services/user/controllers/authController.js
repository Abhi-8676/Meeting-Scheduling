...
exports.googleAuthCallback = (req, res) => {
  try {
    ...
    if (!req.user) {
      return res.redirect(
        `${process.env.CLIENT_URL || "https://meeting-scheduling-eight.vercel.app"}/login?error=OAuthFailed`
      );
    }
    ...
    const clientUrl = process.env.CLIENT_URL || "https://meeting-scheduling-eight.vercel.app";
    ...
    res.redirect(
      `${process.env.CLIENT_URL || "https://meeting-scheduling-eight.vercel.app"}/login?error=ServerError`
    );
  }
};